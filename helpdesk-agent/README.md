...
```bash
cd helpdesk-agent
uv sync                          # workspace venv: agent + UI + dev tools
...
```bash
uv run pytest tests/             # XPIA tests (needs an LLM provider)
uv run helpdesk-ui               # boots the demo web app on :8000
```

> 💡 **Plain pip?** `pip install -e '.[ui]'` from this directory
> gives you the agent + the web UI in one venv. Add `,azure` for
> Entra ID. Without `[ui]` you get just the agent and its tests.

A plain install pulls in RAMPART (from
[microsoft/RAMPART](https://github.com/microsoft/RAMPART)), Microsoft
Agent Framework, `pytest`, and `pytest-asyncio`. The optional extras:

- `[ui]` — adds the FastAPI + React demo (`helpdesk-ui` console script).
- `[azure]` — adds `azure-identity`, only required for Entra ID.
